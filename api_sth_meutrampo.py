...
# Constants for STH
IP_ADDRESS = "4.172.212.188"   # ajuste se necessário
PORT_STH = 8666
DASH_HOST = "0.0.0.0"
...
def get_sth_attribute(entity_type: str, attribute: str, lastN: int):
    ...
    for entry in values:
        if isinstance(entry, dict):
            val = entry.get('attrValue') or entry.get('value')
...
def update_store(n, stored_data):
    raw_motion = get_sth_attribute('Device', 'motion', lastN)
    raw_state = get_sth_attribute('Device', 'state', 50)
    ...
    for entry in raw_motion:
        ...
        if val == '1' or val.lower().startswith('1') or val.lower().startswith('t'):
            counts[minute] += 1
...
def update_graph(store):
    ...
    trace_motion = go.Bar(x=x, y=y, name='Movimentos (count)', marker=dict())
    ...
    if store.get('state_ts') and store.get('state_vals'):
        ...
        ys = store['state_vals']
        trace_state = go.Scatter(x=xs, y=ys, mode='lines+markers', name='Estado Lamp (1=ON,0=OFF)', yaxis='y2')
        data.append(trace_state)
        layout = go.Layout(title='Movimentos por Minuto (fuso: ' + str(USER_TZ) + ')',
                           xaxis_title='Minuto', yaxis_title='Movimentos',
                           yaxis2=dict(title='Estado Lamp (0/1)', overlaying='y', side='right', range=[-0.1,1.1]),
                           barmode='group')
    else:
        layout = go.Layout(title='Movimentos por Minuto (fuso: ' + str(USER_TZ) + ')',
                           xaxis_title='Minuto', yaxis_title='Movimentos', barmode='group')
